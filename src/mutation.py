...
def mutation_among_bad_rows(entity: list, insert_list_indexes: list, bad_rows_indexes: list, row_column_flag: bool) -> None:
    if row_column_flag == True:
        numbers = []
        for item in bad_rows_indexes:
            numbers += list(range(item * 9, item * 9 + 9))

        for item in insert_list_indexes:
            if item[0] in bad_rows_indexes:
                numbers.remove(item[0] * 9 + item[1])
